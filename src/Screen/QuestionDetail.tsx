...
const chartConfig = {
  ...
  const activityData = [
    { id: '1', userLeft: 'Hella', userRight: 'Robin', leftValue: '₹9', rightValue: '', color: 'blue' },
    { id: '2', userLeft: 'Hella', userRight: 'Robin+', leftValue: '₹5', rightValue: '₹5', color: '#d3d3d3' },
    { id: '3', userLeft: 'Hella', userRight: 'Robin+', leftValue: '₹2', rightValue: '₹8', color: '#d3d3d3' },
    { id: '4', userLeft: 'Hella', userRight: 'Robin+', leftValue: '₹4', rightValue: '₹6', color: '#d3d3d3' },
  ];
  ...
  const renderItem = ({ item }) => (
    ...
    alignItems: 'center'}}>
      <View style={styles.progressBarContainer}>
        ...
        <View style={[styles.progressBarRight, { flex: item.rightValue ? 1 : 0 }]}>
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
          <Image source={require('../Assests/ipl.jpg')} style={{ height: 100, width: 100, resizeMode: 'contain' }} />
          <Text style={{ padding: 10, fontSize: 15, fontWeight: 'bold' }}>{route?.params?.prediction?.match}</Text>
          <Text style={{ padding: 10, fontSize: 12, fontWeight: 'bold' }}>{"IPL    T20    Cricket"}</Text>
          <Text style={{ padding: 10, fontSize: 12, fontWeight: 'bold' }}>{route?.params?.prediction?.details}</Text>
        </View>

        <View style={{ marginLeft: 20, marginTop: 25 }}>
          <Text style={{ color: '#000000', fontSize: 12, fontWeight: 'bold', paddingBottom: 10 }}>THE MARKET TRENDS / CHARTS</Text>
          <LineChart
            data={data}
        ...
        <View style={styles.container1}>
          {options.map((option, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.button1,
                selectedOptions.includes(option) ? styles.selectedButton1 : styles.unselectedButton1,
              ]}
            >
              <Text style={[
                styles.buttonText1,
                selectedOptions.includes(option) ? styles.selectedText1 : styles.unselectedText1,
              ]}>
                {option}
              </Text>
            </TouchableOpacity>
        ...
        <View style={styles.tabContainer}>
          ...
          >
            <Text style={[styles.tabButtonText, activeTab === 'Activity' && styles.activeTabButtonText]}>Activity</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.tabButton, activeTab === 'Order Book' && styles.activeTabButton]}
            onPress={() => setActiveTab('Order Book')}
          >
            <Text style={[styles.tabButtonText, activeTab === 'Order Book' && styles.activeTabButtonText]}>Order Book</Text>
          </TouchableOpacity>
        </View>
      ...
      ListFooterComponent={<View style={{height:40,justifyContent:'center',alignItems:'center',borderWidth:0.5,borderRadius:10,width:'90%',margin:10}}><Text style={styles.showAll}>Show all →</Text></View>}
    />
      ...
      <View style={styles.eventDetailsContainer2}>
        ...
        <Text style={styles.subSectionText2}>Information about source of truth</Text>
